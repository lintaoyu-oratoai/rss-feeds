"""Generate RSS feed for the ElevenLabs Blog (https://elevenlabs.io/blog).

Next.js + Tailwind. Index page exposes ~40+ post links under /blog/<slug>.
Per-post pages have og:title and og:description; date is hit-or-miss in
metadata so we cache first-seen timestamps as fallback.
"""
...
FEED_NAME = "elevenlabs"
BLOG_URL = "https://elevenlabs.io/blog"
POST_PREFIX = "/blog/"
FEED_TITLE = "ElevenLabs Blog"
FEED_DESCRIPTION = "Voice AI, text-to-speech, and product updates from ElevenLabs"

SKIP_SEGMENTS = ("/category/", "/tag/", "/author/", "/topic/", "/page/")
...
def discover_posts(blog_url: str) -> list[dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if any(seg in href for seg in SKIP_SEGMENTS):
            continue
        slug = href[len(POST_PREFIX):].split("/", 1)[0].split("?", 1)[0].split("#", 1)[0]
        # Skip pagination URLs like /blog/2, /blog/34
        if slug.isdigit():
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate ElevenLabs Blog RSS feed")
